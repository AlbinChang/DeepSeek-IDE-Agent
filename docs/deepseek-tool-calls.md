...
> 来源：[DeepSeek API Docs - Tool Calls](https://api-docs.deepseek.com/zh-cn/guides/tool_calls)
...
3. 用户：调用 function `get_weather({location: 'Hangzhou'})`，并将结果传给模型
...
`object` 定义一个包含键值对的深层结构，其中 `properties` 定义了对象中每个键（属性）的 schema。

**关键约束**：每个 `object` 的所有属性均需设置为 `required`，且 `object` 中 `additionalProperties` 属性必须为 `false`。
...
**相关链接**：[JSON Output](https://api-docs.deepseek.com/zh-cn/guides/json_mode) | [上下文硬盘缓存](https://api-docs.deepseek.com/zh-cn/guides/kv_cache)
